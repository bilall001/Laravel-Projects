                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        ...
                                        @if (
                                            $viewer &&
                                                in_array($viewer->role, ['admin', 'super_admin']) &&
                                                !in_array($user->role, ['admin', 'super_admin']) &&
                                                $viewer->id !== $user->id)
                                            <form action="{{ route('impersonate.start', $user->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                        ...
                                        {{-- View --}}
                                        <button class="btn btn-sm btn-light view-user-btn" data-name="{{ $user->name }}"
                                            data-username="{{ $user->username }}" data-email="{{ $user->email }}"
                                            data-role="{{ ucfirst($user->role) }}" title="View">
                                            <i class="fas fa-eye text-primary"></i>
                                        </button>

                                        {{-- Edit + Delete --}}
                                        @if (auth()->user()->role === 'admin')
                                            <button class="btn btn-sm btn-light edit-user-btn"
                                                data-id="{{ $user->id }}"
