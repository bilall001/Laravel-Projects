{{-- Controller must send: $teams, $developers --}}
  ...
  <div class="card shadow-sm border-0">
    ...
    <div class="card-body table-responsive">
      <table class="table table-hover mb-0">
          ...
          <tr>
            ...
            <td>
              @foreach ($team->developers as $dev)
                <span class="badge bg-info text-dark">{{ $dev->user->name ?? 'Unnamed' }}</span>
              @endforeach
            </td>
            ...
            <td class="text-center">
              @php
                $teamPayload = [
                  ...
                  'users'        => $team->developers->map(fn($d) => [
                    'id'   => $d->id,                     // Developer.id
        ...
        @empty
          <tr><td colspan="5" class="text-center text-muted">No teams found.</td></tr>
        @endforelse
        </tbody>
      </table>
    </div>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function addUserToList(devId, name) {
    if ([...selectedUsersList.querySelectorAll("input[name='users[]']")]
        .some(input => input.value == devId)) return;

    const li = document.createElement("li");
    li.className = "list-group-item d-flex justify-content-between align-items-center";
    li.innerHTML = `${name}
      <input type="hidden" name="users[]" value="${devId}">
  ...
  addUserBtn.addEventListener('click', () => {
    const devId = userSelect.value;
    const name = userSelect.options[userSelect.selectedIndex]?.dataset.name || '';
    if (!devId) return;
    addUserToList(devId, name);
    userSelect.value = '';
  });
...
selectedUsersList.addEventListener('click', (e) => {
    if (e.target.classList.contains('remove-user')) {
        ...
        if (li) li.remove(); // removes both label + hidden input
    }
});
  ...
  editTeamBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (team.users && team.users.length) {
        team.users.forEach(u => addUserToList(u.id, u.name));
      }
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (selectedUsersList.querySelectorAll("input[name='users[]']").length === 0) {
      e.preventDefault();
      formErrors.textContent = 'Please select at least one developer.';
      formErrors.classList.remove('d-none');
      return false;
    }
  });
