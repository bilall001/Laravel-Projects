...
class TeamManagerDashboardController extends Controller
{
    ...
    $clients = Client::whereHas('projects', function($q) use ($teamId) {
        $q->whereIn('team_id', $teamId);
    })->count();

    $projects = Project::whereIn('team_id', $teamId)->count();
    // dd($projects);
    $currentProjects = Project::whereIn('team_id', $teamId)
        ->whereDate('start_date', '<=', now())
        ->whereDate('end_date', '>=', now())
        ->count();

    $monthCompletedProjects = Project::whereIn('team_id', $teamId)
        ->whereMonth('end_date', now()->month)
        ->whereYear('end_date', now()->year)
        ->count();

    $totalIncome = Project::whereIn('team_id', $teamId)->sum('paid_price');
